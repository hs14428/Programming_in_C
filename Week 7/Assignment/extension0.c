...
int main(int argc, char* argv[MAXLEN])
{
   ...
   int  route[MAXCH];
   Dimensions dim;
   Bookcase* bcArr = (Bookcase*)calloc(MAXBC+1, sizeof(Bookcase));
   if (bcArr == NULL) {
...
/* Couldnt think of better name - part two of happy shelf. Checks
if a single colour is split over multiple svs. If so == not happy */
/* Tested? - yes*/
bool happyShelf2(char svs[HEIGHT][WIDTH], Dimensions dim)
{
   int h, h2, w, w2;
...
/* Function that counts num of each colour; if > bookcase
   width, will fail happy bookcase test                   */
/* Tested? - yes*/
bool colourCount(char svs[HEIGHT][WIDTH], Dimensions dim)
{
   int i, h, w;
...
/* Rules/tests:
   Done - Valid move - move only one book at a time - oneMove
   Valid move - move only rightmost book of shelf
   Valid move - cannot insert book in middle of shelf
   Done - Valid move - no. books cannot exceed shelf width - isShelfFull
   Done - Valid colour - only prescribed colours allowed - isBook
   Done - Happy Colour - each shelf only one colour (+empty) - colourCount
   Done - Happy Shelf - All same coloured books on same shelf (similar to Happy Colour) - happyShelf
   Done - Happy Shelf pos - no empty space before book - happyBookPos
   Done - Happy Bookcase - All svs pass Happy Colour fn - happyBookcase
   Done - Convert bookshelf to string to strcmp with other boards - Stringify

   If more no. of one book colour than width == not poss

   Functions:
   Done - Is cell a book - isBook
   Done - Is the shelf empty? - isShelfEmpty
   Done - Is the shelf full? - isShelfFull
   Done - Check colour of book - colourMatch
   Done - Print bookcase/svs
   Import bookcase

   General notes:
   Maze logic for setting max array size and scan first 2 numbers
   Extensions - remove duplicates from child generation
   change svs, maxh, maxw to structure
   change colours to num index

   Move an end book to an end position:
   so i like had a function that went through a row
   and returned the index position of the last book
   if ur moving a book from this row you swap the character in this cell with the last cell of another row
   also had a num books function that checks the number of books on that row and asserted that this changed by 1
*/
