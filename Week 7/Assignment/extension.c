...
void happyInit     (Bookcase bcArr[MAXBC], Dimensions dim);
...
/* Tests the initial bookcase to see if it is happy or full already and
   cannot be changed. Will print "No solution?" if not happy and full */
int testInit(Bookcase bcArr[MAXBC], Dimensions dim)
{
   int h, cnt;
   cnt = 0;
...
/* Prints out initial state and ends the program. Used for checking
   if starting shelves are happy */
void happyInit(Bookcase bcArr[MAXBC], Dimensions dim)
{
   printf("1\n\n");
...
/* Checks if the actual bookcase dimensions match top level file dimensions */
int bcWidthCheck(int cntW, Dimensions dim)
{
   if (cntW > dim.width) {
...
/* Checks if the actual bookcase height match top level file height
   and checks only valid characters are in the bookcase */
void validFile(char str[HEIGHT*WIDTH+1], int cntH, int w, Dimensions dim)
{
   int i;
...
/* Part one of happy shelf. Checks that only the same colour are on a row
   If there are multiple colours on one row == not happy */
bool happyShelf(char svs[HEIGHT][WIDTH], int h, Dimensions dim)
{
   int w;
   if (h < 0 || h > dim.height) {
...
/* Function resets height counter to 0 (i.e. the top) if it has
   reached the end of the rows of shelves, otherwise adds 1 */
int shelfWrap(int h, Dimensions dim)
{
   int h2;
...
/* Checks for duplicate shelves and if shelf being check is a dupe,
   the shelf will be cleared and not added to the array */
bool dupeShelves(Bookcase bcArr[MAXBC], int c, Dimensions dim)
{
   int i;
   char str1[MAXSTR];
...
/* Main function to run the actual bruteforce algorithm */
int bruteForce(Bookcase bcArr[MAXBC], int c, int p, Dimensions dim)
{
   int h, w, h2, w2, cnt;
   for (h = 0; h < dim.height; h++) {
...
/* Function to run the bruteForce algorithm until a happy bookcase is found or if
   the memory limit is reached. Function also saves parental route for printing */
int runHappy(Bookcase bcArr[MAXBC], int route[MAXCH], Dimensions dim)
{
   int i = 0;
   int c = 0;
